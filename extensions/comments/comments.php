...
if ($m == 'edit' && $id > 0)
{
	if ($a == 'update' && $id > 0)
	{
		...
		$time_limit = ($sys['now'] < ($row['com_date'] + $cfg['comments']['time'] * 60)) ? TRUE : FALSE;
		...
		if (mb_strlen($comarray['com_text']) < $cfg['comments']['minsize'])
		{
	...
	$com_limit = ($sys['now'] < ($com['com_date'] + $cfg['comments']['time'] * 60)) ? TRUE : FALSE;
...
if ($a == 'send' && $usr['auth_write'])
{
	...
	if (mb_strlen($rtext) < $cfg['comments']['minsize'])
	{
		cot_error($L['com_commenttooshort'], 'rtext');
	}
	if ($cfg['comments']['commentsize'] && mb_strlen($rtext) > $cfg['comments']['commentsize'])
	{
