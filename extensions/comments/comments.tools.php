...
/* === Hook  === */
foreach (cot_getextensions('admin.comments.first') as $pl)
{
	include $pl;
}
/* ===== */
...
foreach ($sql->fetchAll() as $row)
{
	...
	switch ($row['com_area'])
	{
		...
		case 'weblogs':
			$row['com_url'] = cot_url('module', 'e=weblogs&m=page&id='.$row['com_value'], '#c'.$row['com_id']);
		break;

		case 'gal':
			$row['com_url'] = cot_url('module', 'e=gal&pic='.$row['com_value'], '#c'.$row['com_id']);
		break;
		...
		case 'e_shop':
			$row['com_url'] = cot_url('module', 'e=e_shop&sh=product&productID='.$row['com_value'], '#c'.$row['com_id']);
		break;
...
/* === Hook  === */
foreach (cot_getextensions('admin.comments.tags') as $pl)
{
	include $pl;
}
/* ===== */
