...
if (isset($_POST['rtext']))
{
	...
	if (mb_strlen($rcontact['contact_text']) < $cfg['contact']['minchars'])
	{
	...
	if (!cot_error_found())
	{
		...
		if (in_array($cfg['contact']['save'], array('db','both')))
		{
			$db->insert($db_contact, $rcontact);
		}

		$semail = (!empty($cfg['contact']['email'])) ? $cfg['contact']['email'] : $cfg['adminemail'];
		if (cot_check_email($semail) && in_array($cfg['contact']['save'], array('email','both')))
		{
			...
			$rtextm = cot_rc(empty($cfg['contact']['template']) ? $R['contact_message'] : $cfg['contact']['template'], $context);
