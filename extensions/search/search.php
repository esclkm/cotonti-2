...
$cfg_maxitems = is_numeric($cfg['search']['maxitems']) ? abs(floor($cfg['search']['maxitems'])) : 50;
...
if (($tab == 'pag' || empty($tab)) && cot_extension_active('page') && $cfg['search']['pagesearch'])
{
...
if (($tab == 'frm' || empty($tab)) && cot_extension_active('forums') && $cfg['search']['forumsearch'])
{
...
if (!empty($sq))
{
	...
	if (mb_strlen($sq) < $cfg['search']['minsigns'])
	{
		cot_error($L['plu_querytooshort'].$R['code_error_separator'], '');
	}
	if (count($words) > $cfg['search']['maxwords'])
	{
		cot_error($L['plu_toomanywords'].' '.$cfg['search']['maxwords'].$R['code_error_separator']);
	}
	// Users LIST
	...
	if (($tab == 'pag' || empty($tab)) && cot_extension_active('page') && $cfg['search']['pagesearch'] && !cot_error_found())
	{
		...
		foreach (explode(',', trim($cfg['search']['addfields'])) as $addfields_el)
		{
		...
		$jj = 0;
		/* === Hook - Part 1 === */
		$extp = cot_getextensions('search.page.loop');
		/* ===== */
	...
	if (($tab == 'frm' || empty($tab)) && cot_extension_active('forums') && $cfg['search']['forumsearch'] && !cot_error_found())
	{
