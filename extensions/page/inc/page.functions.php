...
function cot_generate_pagetags($page_data, $tag_prefix = '', $textlength = 0, $admin_rights = null, $pagepath_home = false, $emptytitle = '')
{
	...
	{
		$extp_first = cot_getextensions('pagetags.first');
		$extp_main = cot_getextensions('pagetags.main');
	}
...
function cot_page_import($source = 'POST', $rpage = array(), $auth = array())
{
	...
	if (empty($rpage['page_parser']) || !in_array($rpage['page_parser'], $parser_list) || $rpage['page_parser'] != 'none' && !cot_auth( $rpage['page_parser'], 'any', 'W'))
	{
