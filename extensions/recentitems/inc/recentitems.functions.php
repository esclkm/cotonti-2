...
function cot_build_recentpages($template, $mode = 'recent', $maxperpage = 5, $d = 0, $titlelength = 0, $textlength = 0, $rightprescan = true, $cat = '')
{
	...
	{
		$whitelist = array();
		foreach (preg_split('#\r?\n#', $cfg['recentitems']['whitelist']) as $c)
		{
	...
	if (!empty($cfg['recentitems']['blacklist']))
	{
		$blacklist = array();
		foreach (preg_split('#\r?\n#', $cfg['recentitems']['blacklist']) as $c)
		{
	...
	if ($mode == 'recent')
	{
		...
		$totalrecent['pages'] = $cfg['recentitems']['maxpages'];
	}
	else
	{
	...
	/* === Hook - Part1 === */
	$extp = cot_getextensions('recentitems.recentpages.tags');
	/* ===== */
