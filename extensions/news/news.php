...
$categories = explode(',', $cfg['news']['category']);
$jj = 0;
$cats = array();
foreach ($categories as $v)
{
	...
	if (isset($structure['page'][$v[0]]))
	{
		...
		$v[1] = ((int)$v[1] > 0) ? $v[1] : (int)$cfg['news']['maxpages'];

		$_GET[$v[0].'d'] = (empty($c) || ($jj == 0) || $cfg['news']['syncpagination']) ? $_GET['d'] : $_GET[$v[0].'d'];
...
if (count($cats) > 0)
{
	...
	$news_first_extp = cot_getextensions('news.first');
	/* === Hook - Part1 : Set === LOOP === */
	$news_extp = cot_getextensions('news.loop');
	/* === Hook - Part1 : Set === TAGS === */
	$news_tags_extp = cot_getextensions('news.tags');
	/* ===== */
	$catn = 0;
	foreach ($cats as $k => $v)
	{
		...
		// Cache for guests
		if ($usr['id'] == 0 && $cache && (int) $cfg['news']['cache_ttl'] > 0)
		{
			$news_cache_id = "$theme.$lang.$cat." . $v[3]['d']; // Includes theme, lang, category and current page
			$news_html = $cache->disk->get($news_cache_id, 'news', (int) $cfg['news']['cache_ttl']);
