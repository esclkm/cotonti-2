...
$pagenav = cot_pagenav('admin', 't=polls'.$poll_filter, $d, $totalitems, $cfg['maxrowsperpage'], 'd', '', $cfg['jquery'] && $cfg['turnajax']);
...
/* === Hook - Part1 : Set === */
$extp = cot_getextensions('polls.admin.loop');
/* ===== */
...
foreach ($sql_polls->fetchAll() as $row)
{
	...
	$t->assign(array(
		...
		'ADMIN_POLLS_ROW_POLL_URL' => cot_url('admin', 't=polls'.$poll_filter.'&n=options&d='.$durl.'&id='.$row['poll_id']),
		'ADMIN_POLLS_ROW_POLL_TEXT' => htmlspecialchars($row['poll_text']),
		'ADMIN_POLLS_ROW_POLL_URL_LCK' => cot_url('admin', 't=polls'.$poll_filter.'&a=lock&id='.$id.'&'.cot_xg()),
		'ADMIN_POLLS_ROW_POLL_TOTALVOTES' => $totalvotes,
		'ADMIN_POLLS_ROW_POLL_LOCKED' => ($row['poll_state']) ? '[-] ' : '',
		'ADMIN_POLLS_ROW_POLL_URL_DEL' => cot_url('admin', 't=polls'.$poll_filter.'&a=delete&id='.$id.'&'.cot_xg()),
		'ADMIN_POLLS_ROW_POLL_URL_LCK' => cot_url('admin', 't=polls'.$poll_filter.'&a=lock&id='.$id.'&'.cot_xg()),
		'ADMIN_POLLS_ROW_POLL_URL_RES' => cot_url('admin', 't=polls'.$poll_filter.'&a=reset&d='.$durl.'&id='.$id.'&'.cot_xg()),
		'ADMIN_POLLS_ROW_POLL_URL_BMP' => cot_url('admin', 't=polls'.$poll_filter.'&a=bump&id='.$id.'&'.cot_xg()),
...
foreach($variants as $val)
{
	...
	$t->assign(array(
		'ADMIN_POLLS_ROW_FILTER_VALUE' => cot_url('admin', 't=polls'.$val[1]),
...
$t->assign(array(
	'ADMIN_POLLS_CONF_URL' => cot_url('admin', 't=config&n=edit&o=extension&p=polls'),
