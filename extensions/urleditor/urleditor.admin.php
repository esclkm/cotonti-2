...
list($usr['auth_read'], $usr['auth_write'], $usr['isadmin']) = cot_auth('urleditor', 'any');
cot_block($usr['isadmin']);

$t = new XTemplate(cot_tplfile('urleditor.admin', 'module', true));
...
/* === Hook === */
foreach (cot_getextensions('admin.urls.first') as $pl)
{
	include $pl;
}
/* ===== */
...
if ($a == 'save' && is_writable('./datas/urltrans.dat'))
{
	...
	/* === Hook === */
	foreach (cot_getextensions('admin.urls.save') as $pl)
	{
		include $pl;
	}
	/* ===== */
...
if (is_readable('./datas/urltrans.dat'))
{
	...
	$ii = 0;
	/* === Hook - Part1 : Set === */
	$extp = cot_getextensions('admin.urls.loop');
	/* ===== */
...
/* === Hook  === */
foreach (cot_getextensions('admin.urls.tags') as $pl)
{
	include $pl;
}
/* ===== */
