...
function cot_ratings_display($ext_name, $code, $cat = '', $readonly = false)
{
	...
	// Check permissions
	list($auth_read, $auth_write, $auth_admin) = cot_auth('ratings', 'any');
	...
	if ($already_voted && !$cfg['ratings']['ratings_allowchange'])
	{
		return array($rating_fancy, $rating_cntround, $rating_raters_count);
	}
	...
	// Render voting form
	$vote_block = ($auth_write && (!$already_voted || $cfg['ratings']['ratings_allowchange'])) ? 'NOTVOTED.' : 'VOTED.';
	for ($i = 1; $i <= 10; $i++)
	{
...
function cot_ratings_enabled($ext_name, $cat = '', $item = '')
{
		...
		|| isset($cfg[$ext_name]['enable_ratings']))
	{
		...
		{
			return (bool) $cfg[$ext_name]['enable_ratings'];
		}
	}
	return true;
}
