...
$t = new XTemplate(cot_tplfile('admin.rights', 'system'));
...
/* === Hook === */
foreach (cot_getextensions('admin.rights.first') as $ext)
{
	include $ext;
}
/* ===== */
...
if ($a == 'update')
{
	...
	/* === Hook === */
	foreach (cot_getextensions('admin.rights.update') as $ext)
	{
		include $ext;
	}
	/* ===== */
...
foreach (cot_getextensions('admin.rights.main') as $ext)
{
	include $ext;
}
/* ===== */
...
while ($row = $sql->fetch())
{
	...
	{
		$t->assign('RIGHTS_SECTION_TITLE', $L['Module'].' '.$cot_extensions[$area]['title'].' '.mb_strtolower($L['Structure']));
		$t->parse('MAIN.RIGHTS_SECTION');
	}
...
/* === Hook for the extensions === */
foreach (cot_getextensions('admin.rights.end') as $ext)
{
	include $ext;
}
/* ===== */
...
/* === Hook === */
foreach (cot_getextensions('admin.rights.tags') as $ext)
{
	include $ext;
}
/* ===== */
