...
/* === Hook === */
foreach (cot_getextensions('admin.users.first') as $ext)
{
	include $ext;
}
/* ===== */
...
if($n == 'add')
{
	...
	/* === Hook === */
	foreach (cot_getextensions('admin.users.add.first') as $ext)
	{
		include $ext;
	}
	/* ===== */
	...
	if (!cot_error_found())
	{
		...
		/* === Hook === */
		foreach (cot_getextensions('admin.users.add') as $ext)
		{
			include $ext;
		}
		/* ===== */
...
elseif($n == 'edit')
{
	if($a == 'update')
	{
		...
		/* === Hook === */
		foreach (cot_getextensions('admin.users.update') as $ext)
		{
			include $ext;
		}
		/* ===== */
	...
	elseif($a == 'delete' && $g > 5)
	{
		...
		/* === Hook === */
		foreach (cot_getextensions('admin.users.delete') as $ext)
		{
			include $ext;
		}
		/* ===== */
	...
	else
	{
		...
		/* === Hook === */
		foreach (cot_getextensions('admin.users.edit.tags') as $ext)
		{
			include $ext;
		}
		/* ===== */
...
if(!isset($showdefault) || $showdefault == true)
{
	...
	/* === Hook === */
	foreach (cot_getextensions('admin.users.add.tags') as $ext)
	{
		include $ext;
	}
	/* ===== */
...
/* === Hook  === */
foreach (cot_getextensions('admin.users.tags') as $ext)
{
	include $ext;
}
/* ===== */
