...
if ($cfg['cleaner']['logprune'] > 0)
{
	...
	{
		cot_log('Cleaner plugin deleted '.$db->affectedRows.' log entries older than '.$cfg['cleaner']['logprune'].' days', 'adm');
	}
}

if ($cfg['cleaner']['refprune'] > 0 && $cot_plugins['tools']['referers'])
{
	$timeago = $sys['now'] - ($cfg['cleaner']['refprune'] * 86400);
	$db->delete($db_referers, "ref_date < $timeago");
	if ($db->affectedRows > 0)
	{
		cot_log('Cleaner plugin deleted '.$db->affectedRows.' referers entries older than '.$cfg['cleaner']['refprune'].' days', 'adm');
	}
}
...
if (cot_module_active('pm'))
{
	...
	if ($cfg['cleaner']['pmnotread'] > 0)
	{
		...
		if ($db->affectedRows > 0)
		{
			cot_log("Cleaner plugin deleted ".$db->affectedRows." PM not read since ".$cfg['cleaner']['pmnotread']." days", 'adm');
		}
	}
	...
	if ($cfg['cleaner']['pmnotarchived'] > 0)
	{
		...
		if ($db->affectedRows > 0)
		{
			cot_log("Cleaner plugin deleted ".$db->affectedRows." PM not archived since ".$cfg['cleaner']['pmnotarchived']." days", 'adm');
		}
	}
	...
	if ($cfg['cleaner']['pmold'] > 0)
	{
		...
		if ($deleted > 0)
		{
			cot_log("Cleaner plugin deleted ".$deleted." PM older than ".$cfg['cleaner']['pmold']." days", 'adm');
		}
	}
}
