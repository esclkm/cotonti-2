...
if (!$ckeditor_timestamp)
    $ckeditor_timestamp = ckeditor_max_timestamp($cfg['modules_dir'] . '/ckeditor/lib');

// Main CKEditor file
cot_rc_link_footer($cfg['modules_dir'] . '/ckeditor/lib/ckeditor.js?'.$ckeditor_timestamp);

// Load preset and connector
if ($usr['id'] > 0)
{
    $preset_name = 'group_' . $usr['maingrp'];
    if (!file_exists($cfg['modules_dir'] . "/ckeditor/presets/ckeditor.$preset_name.set.js"))
    {
        $preset_name = 'default';
    }
}
...
{
    $preset_name = file_exists($cfg['modules_dir'] . "/ckeditor/presets/ckeditor.group_1.set.js") ? 'group_1'
        : 'default';
}
cot_rc_link_footer($cfg['modules_dir'] . "/ckeditor/presets/ckeditor.$preset_name.set.js?".$ckeditor_timestamp);
