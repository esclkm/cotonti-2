...
if ($cfg['jquery'])
{
	// Language selection
	global $lang;
	$mkup_lang = $cfg['modules_dir']."/markitup/lang/$lang.lang.js";
	if (!file_exists($mkup_lang))
	{
		$mkup_lang = $cfg['modules_dir'].'/markitup/lang/en.lang.js';
	}
	...
	// Load resources
	$mkup_skin_path = $cfg['modules_dir'] . '/markitup/skins/' . $cfg['plugin']['markitup']['skin'] . '/style.css';
	$mkup_skin = cot_rc('code_rc_css_file', array('url' => $mkup_skin_path));
	$mkup_style_path = $cfg['modules_dir'] . '/markitup/style.css';
	...
	cot_rc_link_footer($cfg['modules_dir'] . '/markitup/js/jquery.markitup.min.js');
	cot_rc_link_footer($mkup_lang);
	cot_rc_link_footer($cfg['modules_dir'] . "/markitup/js/{$mkup_set}.set.js");

	if ($cfg['plugin']['markitup']['chili'])
	{
		cot_rc_link_footer($cfg['modules_dir'].'/markitup/js/chili.js');
	}
