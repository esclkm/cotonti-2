...
function htmlpurifier_filter($value, $name)
{
	...
	if ($sys['parser'] == 'html')
	{
		...
		if (is_null($purifier))
		{
			define('HTMLPURIFIER_PREFIX', $cfg['modules_dir'] . '/htmlpurifier/lib/standalone');
			require_once $cfg['modules_dir'] . '/htmlpurifier/lib/HTMLPurifier.standalone.php';
			...
			if ($usr['id'] > 0)
			{
				...
				if (!file_exists($cfg['modules_dir'] . "/htmlpurifier/presets/htmlpurifier.$preset_name.preset.php"))
				{
					$preset_name = 'default';
				}
			}
			...
			require_once  $cfg['modules_dir'] . "/htmlpurifier/presets/htmlpurifier.$preset_name.preset.php";
			/* config extension */
