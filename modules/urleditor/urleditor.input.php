...
if (!is_array($cot_urltrans))
{
	...
	{
		$urltrans_preset = file_exists('./datas/' . $cfg['plugin']['urleditor']['preset'] . '.dat') ? './datas/' . $cfg['plugin']['urleditor']['preset'] . '.dat' : $cfg['modules_dir'] . '/urleditor/presets/' . $cfg['plugin']['urleditor']['preset'] . '.dat';
	}
...
if(!in_array($cfg['plugin']['urleditor']['preset'], array('custom', 'none')))
{
	...
	elseif(file_exists($cfg['modules_dir'] . '/urleditor/presets/' . $cfg['plugin']['urleditor']['preset'] . '.functions.php'))
	{
		require_once $cfg['modules_dir'] . '/urleditor/presets/' . $cfg['plugin']['urleditor']['preset'] . '.functions.php';
	}
}
